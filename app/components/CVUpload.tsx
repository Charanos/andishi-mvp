...
export default function CVUpload({
  ...
  const handleFileSelect = async (file: File) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      setToast({
        type: "error",
        message: "Please upload a valid CV file (PDF, DOC, DOCX, or TXT)",
      });
  ...
  return (
    <div>
      <label className="text-gray-700 dark:text-gray-300 text-sm font-medium mb-2 flex items-center">
        <FaFileUpload className="mr-2 text-blue-600 dark:text-blue-400" />
        {label}
      </label>
      ...
      <div className="space-y-3">
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center space-y-3">
            {isUploading ? (
              <FaSpinner className="text-2xl text-blue-500 dark:text-blue-400 animate-spin" />
            ) : (
        ...
        {value && !isUploading && (
          <div className="relative">
            <div className="flex items-center space-x-3 p-3 bg-gray-100 dark:bg-black/30 border border-gray-300 dark:border-gray-600 rounded-lg">
              <FaFile className="text-2xl text-blue-600 dark:text-blue-400" />
              <div className="flex-1 min-w-0">
                <p className="text-gray-900 dark:text-gray-300 text-sm font-medium">CV uploaded</p>
                <p className="text-gray-600 dark:text-gray-500 text-xs truncate">{value}</p>
              </div>
              <button
                type="button"
